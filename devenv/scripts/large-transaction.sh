...
export CC_SRC_PATH="github.com/chaincode/largeTransaction/"
# channel name defaults to "mychannel" if no input is provided
CHANNEL_NAME=largedatachannel
NUMBER_OF_TRANSACTIONS=40

value=$(<../data/sample-data.txt)
...
/$CHANNEL_NAME.tx --tls $CORE_PEER_TLS_ENABLED --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/orderers\
...
sudo docker exec -e "CORE_LOGGING_LEVEL=INFO" -e "CORE_PEER_LOCALMSPID=Org1MSP" -e "CORE_PEER_MSPCONFIGPATH=$CORE_PEER_MSPCONFIGPATH" -e "CORE_PEER_ADDRESS=$CORE_PEER_ADDRESS" cli peer chaincode install -n largeTransaction -l golang -v 1.0 -p ${CC_SRC_PATH} 

sleep 30

arg='{"Args":["init","a", "'$value'"]}'
echo $arg

sudo docker exec -e "CORE_LOGGING_LEVEL=INFO" -e "CORE_PEER_LOCALMSPID=Org1MSP" -e "CORE_PEER_MSPCONFIGPATH=$CORE_PEER_MSPCONFIGPATH" -e "CORE_PEER_ADDRESS=$CORE_PEER_ADDRESS" cli peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile /opt/gopath/src\
...
-cert.pem -C $CHANNEL_NAME -n largeTransaction -l golang -v 1.0 -c "$arg"
...
/tlsca.example.com-cert.pem -C $CHANNEL_NAME -n largeTransaction -c '{"Args":["invoke","a"]}'
n=`expr $n + 1`
echo $n
done
exit 0
