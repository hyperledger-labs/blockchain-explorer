...
describe('Chaincodes', () => {
  ...
  test('Simulate Channel Name filterMethod should have one result when given a value of mychannel', () => {
    const { wrapper } = setup();
    wrapper.find('ThComponent').findWhere(n => n.key() === '2-channelName').find('input').simulate('change', {target: {value: 'mychannel'}});
    expect(wrapper.find(ReactTable).find('TrGroupComponent').length).toBe(1);
  });

  test('Simulate Path filterMethod should have one result when given a value of github', () => {
    const { wrapper } = setup();
    wrapper.find('ThComponent').findWhere(n => n.key() === '3-path').find('input').simulate('change', {target: {value: 'github'}});
    expect(wrapper.find(ReactTable).find('TrGroupComponent').length).toBe(1);
  });

  test('Simulate Transaction Count filterMethod should have one result when given a value of 33', () => {
    const { wrapper } = setup();
    wrapper.find('ThComponent').findWhere(n => n.key() === '4-txCount').find('input').simulate('change', {target: {value: '33'}});
    expect(wrapper.find(ReactTable).find('TrGroupComponent').length).toBe(1);
  });

  test('Simulate Version filterMethod should have one result when given a value of 1', () => {
    const { wrapper } = setup();
    wrapper.find('ThComponent').findWhere(n => n.key() === '5-version').find('input').simulate('change', {target: {value: '1'}});
    expect(wrapper.find(ReactTable).find('TrGroupComponent').length).toBe(1);
  });
